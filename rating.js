(function () {
	...
	// ✅ Guard: если запускают в Node (нет window/document) — выходим без ошибки
	...
	// Singleflight: один запрос на один card.id
	...
	function rating_kp_imdb(card, render) {
		...
		function doneInflight() {
			delete __inflight[params.id];
		}

		function getRating() {
			var cached = _getCache(params.id);
			if (cached) {
				_showRating(cached);
				doneInflight();
				return;
		...
		function searchFilm() {
			...
			network.silent(
				...
				function (json) {
					if (json.items && json.items.length) chooseFilm(json.items);
						...
						network.silent(
							...
							function () {
								var fail = _setCache(params.id, { kp: null, imdb: null, ok: false }, TTL_FAIL);
								_showRating(fail);
								doneInflight();
							},
							false,
				...
				function () {
					var fail2 = _setCache(params.id, { kp: null, imdb: null, ok: false }, TTL_FAIL);
					_showRating(fail2);
					doneInflight();
				},
				false,
		...
		function chooseFilm(items) {
			if (!items || !items.length) {
				var nf = _setCache(params.id, { kp: null, imdb: null, ok: false }, TTL_FAIL);
				_showRating(nf);
				doneInflight();
				return;
			}
			...
			if (!(cards.length == 1 && is_sure)) {
				var unsure = _setCache(params.id, { kp: null, imdb: null, ok: false }, TTL_FAIL);
				_showRating(unsure);
				doneInflight();
				return;
			}
			...
			network["native"](
				...
				function (str) {
					...
					if (parsed) {
						var ok = _setCache(params.id, { kp: parsed.kp, imdb: parsed.imdb, ok: true }, TTL_OK);
						_showRating(ok);
						doneInflight();
						return;
					}
			...
			function base_search(kpId) {
				...
				network.silent(
					...
					function (data) {
						...
						var saved = _setCache(params.id, { kp: kp, imdb: imdb, ok: okFlag }, ttl);
						_showRating(saved);
						doneInflight();
					},
					function () {
						var fail3 = _setCache(params.id, { kp: null, imdb: null, ok: false }, TTL_FAIL);
						_showRating(fail3);
						doneInflight();
					},
					false,
		...
		function _setCache(movieId, data, ttl) {
			var now = Date.now();
			var cache = Lampa.Storage.cache(CACHE_KEY, CACHE_LIMIT, {});
			data = data || {};
			data.timestamp = now;
		...
		function _showRating(data) {
			...
			// Проверка “всё ещё та же карточка”
			try {
				...
				if (current && current.activity && current.activity.data && current.activity.data.movie) {
					...
					if (currentId && currentId !== params.id) return;
				}
			} catch (e) {}
			...
			$('.rate--imdb', render).removeClass('hide').find('> div').eq(0).text(imdbText);
			$('.rate--kp', render).removeClass('hide').find('> div').eq(0).text(kpText);
		}
	}
	...
	function startPlugin() {
		...
		Lampa.Listener.follow('full', function (e) {
			...
			if (!$('.rate--kp', render).hasClass('hide')) return;
