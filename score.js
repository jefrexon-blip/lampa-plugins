(function() {
  ...
  (function() {
    ...
    function cleanKinopoiskTitle(value) {
      return cleanTitle(value).replace(/^[ \/\\]+/, "").replace(/[ \/\\]+$/, "").replace(/\+( *[+\/\\])+/g, "+").replace(/([+\/\\] *)+\+/g, "+").replace(/( *[\/\\]+ *)+/g, "+");
    }
    function legacyNormalizeTitle(value) {
      return cleanTitle(String(value || "").toLowerCase().replace(/[\-\u2010-\u2015\u2E3A\u2E3B\uFE58\uFE63\uFF0D]+/g, "-").replace(/\u0451/g, "е"));
    }
    ...
    function chooseLegacyCandidate(items, movie) {
      ...
      if (cards.length && original) {
        tmp = cards.filter(function(item) {
          return containsTitle(item.orig_title || item.nameOriginal, original) || containsTitle(item.en_title || item.nameEn, original) || containsTitle(item.title || item.ru_title || item.nameRu, original);
        });
        if (tmp.length) {
          cards = tmp;
          sure = true;
      ...
      if (cards.length && title) {
        tmp = cards.filter(function(item) {
          return containsTitle(item.title || item.ru_title || item.nameRu, title) || containsTitle(item.en_title || item.nameEn, title) || containsTitle(item.orig_title || item.nameOriginal, title);
        });
        if (tmp.length) {
          cards = tmp;
          sure = true;
      ...
      if (cards.length === 1 && sure && !imdbSure) {
        ...
        if (sure) {
          sure = false;
          if (original) {
            sure = sure || equalTitle(cards[0].orig_title || cards[0].nameOriginal, original) || equalTitle(cards[0].en_title || cards[0].nameEn, original) || equalTitle(cards[0].title || cards[0].ru_title || cards[0].nameRu, original);
          }
          if (title) {
            sure = sure || equalTitle(cards[0].title || cards[0].ru_title || cards[0].nameRu, title) || equalTitle(cards[0].en_title || cards[0].nameEn, title) || equalTitle(cards[0].orig_title || cards[0].nameOriginal, title);
          }
        }
      }
    ...
    function findKinopoiskIdLegacy(movie, apiRoot, headers, done, fail) {
      ...
      if (imdbId) {
        requestJson(Lampa.Utils.addUrlComponent(apiRoot + "api/v2.2/films", "imdbId=" + encodeURIComponent(imdbId)), headers, function(json) {
          ...
          var kpId = selected && (selected.kp_id || selected.kinopoisk_id || selected.kinopoiskId || selected.filmId);
          if (kpId) done({ kpId: kpId, headers: headers, apiRoot: apiRoot });
          else requestJson(searchUrl, headers, resolveList, fail, 6e3);
        }, function() {
          requestJson(searchUrl, headers, resolveList, fail, 6e3);
        }, 6e3);
      } else {
        requestJson(searchUrl, headers, resolveList, fail, 6e3);
      }
