...
(function () {
  ...
  function css() {
    ...
    style.type = 'text/css';
    style.textContent = '' +
      '.cine-score{display:flex;flex-wrap:wrap;gap:12px;margin:18px 0 0;align-items:stretch;}' +
      '.cine-score--compact{gap:8px;}' +
      '.cine-score__item{min-width:114px;display:flex;flex-direction:column;justify-content:center;padding:14px 16px;border-radius:18px;background:linear-gradient(180deg,rgba(255,255,255,.09),rgba(255,255,255,.04));box-shadow:0 14px 30px rgba(0,0,0,.16);border:1px solid rgba(255,255,255,.07);backdrop-filter:blur(18px);}' +
      '.cine-score--compact .cine-score__item{min-width:98px;padding:10px 12px;border-radius:14px;}' +
      '.cine-score__item[data-tone="good"]{background:linear-gradient(180deg,rgba(51,182,112,.24),rgba(17,83,54,.18));border-color:rgba(64,204,129,.34);}' +
      '.cine-score__item[data-tone="mid"]{background:linear-gradient(180deg,rgba(236,180,68,.22),rgba(130,92,22,.18));border-color:rgba(246,199,90,.28);}' +
      '.cine-score__item[data-tone="bad"]{background:linear-gradient(180deg,rgba(233,94,94,.22),rgba(116,34,34,.18));border-color:rgba(236,112,112,.26);}' +
      '.cine-score__item[data-tone="muted"]{opacity:.88;}' +
      '.cine-score__top{display:flex;align-items:center;justify-content:space-between;gap:10px;}' +
      '.cine-score__label{font-size:0.85em;text-transform:uppercase;letter-spacing:.14em;opacity:.75;}' +
      '.cine-score__value{font-size:2.1em;font-weight:700;line-height:1;margin-top:8px;}' +
      '.cine-score--compact .cine-score__value{font-size:1.7em;margin-top:6px;}' +
      '.cine-score__meta{margin-top:8px;font-size:.88em;opacity:.74;}' +
      '.cine-score__item.is-loading .cine-score__value{opacity:.45;}' +
      '.cine-score__actions{display:flex;align-items:center;gap:10px;}' +
      '.cine-score__refresh{display:inline-flex;align-items:center;gap:8px;height:44px;padding:0 16px;border-radius:14px;border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.06);cursor:pointer;}' +
      '.cine-score__refresh svg{width:16px;height:16px;}' +
      '.cine-score__refresh:empty{display:none;}' +
      '.cine-score__refresh.loading svg{animation:cine-score-spin 1s linear infinite;}' +
      '.cine-score__refresh.focus,.cine-score__refresh.hover{background:rgba(255,255,255,.12);border-color:rgba(255,255,255,.22);}' +
      '.cine-score__foot{width:100%;display:flex;justify-content:space-between;align-items:center;gap:12px;font-size:.92em;opacity:.8;}' +
      '.cine-score__status{padding:8px 12px;border-radius:12px;background:rgba(255,255,255,.05);}' +
      '.cine-score__status[data-kind="ok"]{color:#70db98;}' +
      '.cine-score__status[data-kind="fail"]{color:#ff9a9a;}' +
      '.cine-score__stamp{opacity:.68;}' +
      '.cine-score__item.is-pop{animation:cine-score-rise .35s ease;}' +
      '@keyframes cine-score-spin{from{transform:rotate(0deg);}to{transform:rotate(360deg);}}' +
      '@keyframes cine-score-rise{0%{transform:translateY(8px);opacity:.5;}100%{transform:none;opacity:1;}}';
  ...
  function requestJson(url, headers, onSuccess, onError, timeout) {
    ...
    network.clear();
    network.timeout(timeout || 6000);
    network.silent(url, function (json) {
      onSuccess(json);
    }, function () {
      onError();
    }, false, headers ? { headers: headers } : undefined);
  }

  function requestText(url, onSuccess, onError, timeout) {
    var network = createRequest();
    network.clear();
    network.timeout(timeout || 4000);
  ...
  function findKinopoiskId(movie, done, fail) {
    ...
    if (imdbId) {
      requestJson(Lampa.Utils.addUrlComponent(apiRoot + 'api/v2.2/films', 'imdbId=' + encodeURIComponent(imdbId)), headers, function (json) {
        ...
        var kpId = selected && (selected.kinopoiskId || selected.kinopoisk_id || selected.kp_id || selected.filmId);
        if (kpId) done({ kpId: kpId, headers: headers, apiRoot: apiRoot });
        else requestJson(searchUrl, headers, resolveFromItems, fail, 6000);
      }, function () {
        requestJson(searchUrl, headers, resolveFromItems, fail, 6000);
      }, 6000);
    } else {
      requestJson(searchUrl, headers, resolveFromItems, fail, 6000);
    }
  ...
  function createPanel(root, config) {
    ...
    html = '' +
      '<div class="cine-score' + (config.compact ? ' cine-score--compact' : '') + '">' +
        '<div class="cine-score__item" data-key="kp">' +
          '<div class="cine-score__top"><div class="cine-score__label">KP</div></div>' +
          '<div class="cine-score__value">—</div>' +
          '<div class="cine-score__meta">Кинопоиск</div>' +
        '</div>' +
        '<div class="cine-score__item" data-key="imdb">' +
          '<div class="cine-score__top"><div class="cine-score__label">IMDb</div></div>' +
          '<div class="cine-score__value">—</div>' +
          '<div class="cine-score__meta">Internet Movie Database</div>' +
        '</div>' +
        '<div class="cine-score__foot">' +
          '<div class="cine-score__status" data-role="status" data-kind="ok">Загрузка рейтингов</div>' +
          '<div class="cine-score__actions">' +
            '<div class="cine-score__stamp" data-role="stamp"></div>' +
            '<div class="cine-score__refresh selector" data-role="refresh"><svg viewBox="0 0 24 24" fill="none"><path d="M20 12a8 8 0 1 1-2.34-5.66" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><path d="M20 4v5h-5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg><span>Обновить</span></div>' +
          '</div>' +
        '</div>' +
      '</div>';
