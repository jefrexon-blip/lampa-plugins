...
var OnlineMod = window.OnlineMod || (window.OnlineMod = {
...
OnlineMod.Cache = (function () {
  function LRU(max, ttl) {
    this.max = max || 120;
...
OnlineMod.Stats = (function () {
  var KEY = 'online_mod_stats_v2';
  function load(){ return Lampa.Storage.get(KEY, {src:{}}) || {src:{}}; }
  function save(s){ Lampa.Storage.set(KEY, s); }
  function hit(source, ok){
    var s = load();
    if(!s.src[source]) s.src[source] = {ok:0, err:0};
    if(ok) s.src[source].ok++;
    else s.src[source].err++;
    save(s);
  }
  return { hit: hit, get: load };
})();

OnlineMod.UI.errorModal = function(title, message){
  try{
    title = title || 'Источник недоступен';
    message = message || 'Попробуйте позже или смените зеркало/прокси.';
    var html = $('<div class="online_mod_modal">'
      + '<div class="online_mod_modal__title"></div>'
      + '<div class="online_mod_modal__text"></div>'
      + '<div class="online_mod_modal__btns">'
      +   '<div class="online_mod_modal__btn online_mod_modal__btn--ok">OK</div>'
      + '</div>'
      + '</div>');

    html.find('.online_mod_modal__title').text(title);
    html.find('.online_mod_modal__text').text(message);

    var modal = new Lampa.Modal();
    modal.open({ title: '', html: html });

    html.find('.online_mod_modal__btn--ok').on('hover:enter', function(){
      modal.close();
    });
  }catch(e){
    // fallback
    if (typeof Lampa !== 'undefined' && Lampa.Noty) Lampa.Noty.show((title||'') + (message ? (': ' + message) : ''));
  }
};


OnlineMod.UI._buildErrorHint = function(e){
  var raw = '';
  try { raw = (e && (e.message || e.error || e.status || e)) ? String(e.message || e.error || e.status || e) : ''; } catch(_e){ raw = ''; }
  var lower = raw.toLowerCase();

  var status = null;
  try {
    if (e && typeof e.status === 'number') status = e.status;
    else if (e && e.xhr && typeof e.xhr.status === 'number') status = e.xhr.status;
  } catch(_e) { status = null; }


  var is403 = status === 403 || lower.indexOf('403') !== -1 || lower.indexOf('forbidden') !== -1;
  var is429 = status === 429 || lower.indexOf('429') !== -1 || lower.indexOf('too many') !== -1 || lower.indexOf('rate') !== -1;
  var isTimeout = (e && e.statusText === 'timeout') || lower.indexOf('timeout') !== -1 || lower.indexOf('timed out') !== -1;

  var msg = 'Источник не дал поток.\n\nЧто попробовать:\n• Переключить озвучку (чип сверху)\n• Открыть фильтр и выбрать другой сезон/перевод\n• В настройках включить/сменить proxy для источника\n• Если не помогло — попробуйте другое зеркало';

  if (is403) {
    msg = 'Доступ запрещён (403).\n\nЧто попробовать:\n• Включить/сменить proxy для источника\n• Сменить зеркало\n• Переключить озвучку/перевод\n\nЧасто помогает другой прокси.';
  } else if (is429) {
    msg = 'Слишком много запросов (429).\n\nЧто попробовать:\n• Подождать 1–3 минуты\n• Включить/сменить proxy\n• Сменить зеркало\n\nЭто временная блокировка.';
  } else if (isTimeout) {
    msg = 'Источник долго отвечает (timeout).\n\nЧто попробовать:\n• Сменить прокси/зеркало\n• Попробовать другой перевод\n• Повторить позже';
  }

  return msg + (raw ? ('\n\nТех.детали: ' + raw.slice(0,120)) : '');
};



OnlineMod.UI.renderItems = function (component, object, items, ctx) {
  component.reset();

  // optional chips header (season/voice/etc.) - provider passes arrays + callbacks
  try {
    if (ctx && ctx.chips && ctx.chips.length) {
      var header = $('<div class="online_mod_header"></div>');
      var chips = $('<div class="online_mod_chips"></div>');
      ctx.chips.forEach(function (chip) {
        var el = $('<div class="online_mod_chip"></div>');
        if (chip && chip.active) el.addClass('online_mod_chip--active');
        el.text(chip && chip.text ? chip.text : '');
        el.on('hover:enter', function () {
          try { if (chip && chip.onClick) chip.onClick(); } catch (e) {}
        });
        chips.append(el);
      });

      // small stats button (non-intrusive)
      var statsBtn = $('<div class="online_mod_stats_btn" title="Статистика">ⓘ</div>');
      statsBtn.on('hover:enter', function () {
        try { if (OnlineMod && OnlineMod.UI && OnlineMod.UI.showStatsModal) OnlineMod.UI.showStatsModal(); } catch (e) {}
      });

      header.append(chips);
      header.append(statsBtn);
  ...
  items.forEach(function (element) {
    if (element.season) {
      ...
      element.translate_voice = (ctx && ctx.voice_name) || element.translate_voice || '';
    }
    ...
    var hash_file = Lampa.Utils.hash(
      element.season
        ? [element.season, element.season > 10 ? ':' : '', element.episode, object.movie.original_title, (ctx && ctx.voice_name) || ''].join('')
    ...
    // progress bar (if timeline has percent)
    try{
      var perc = 0;
      if (view && typeof view.percent === 'number') perc = view.percent;
      else if (view && typeof view.progress === 'number') perc = view.progress;
      else if (view && typeof view.time === 'number' && typeof view.duration === 'number' && view.duration > 0) perc = (view.time / view.duration) * 100;
      if (perc && perc > 0.5 && perc <= 100) {
        perc = Math.min(100, Math.max(0, perc));
        item.append('<div class="online_mod_progress"><div class="online_mod_progress__bar" style="width:'+perc.toFixed(1)+'%"></div></div>');
      }
    }catch(e){}
    ...
    if (ctx && ctx.source) {
      item.addClass('online_mod_card');
      item.append('<div class="online_mod_badge">' + ctx.source.toUpperCase() + '</div>');
    } else {
      item.addClass('online_mod_card');
    }

    // viewed mark (clean)
    var is_viewed = viewed.indexOf(hash_file) !== -1;
    if (is_viewed) {
      item.addClass('online_mod_card--viewed');
      item.append('<div class="online_mod_viewed_dot" aria-label="Просмотрено"></div>');
    }
    ...
    item.on('hover:enter', function () {
      ...
      ctx.getStream(element, function (el) {
        element.loading = false;
        if (ctx && ctx.source) OnlineMod.Stats.hit(ctx.source, true);

        var first = {
          url: component.getDefaultQuality(el.qualitys, el.stream),
          quality: component.renameQualityMap(el.qualitys),
          subtitles: el.subtitles,
          vast_url: el.vast_url,
          vast_msg: el.vast_msg,
          timeline: el.timeline,
          title: el.season ? el.title : (ctx.select_title || '') + (el.title == (ctx.select_title || '') ? '' : ' / ' + el.title)
        };

        Lampa.Player.play(first);

        if (el.season && Lampa.Platform.version) {
          var playlist = [];
          items.forEach(function (elem) {
            if (elem == el) {
              ...
              var cell = {
                url: function (call) {
                  ctx.getStream(elem, function (e2) {
                    cell.url = component.getDefaultQuality(e2.qualitys, e2.stream);
                    cell.quality = component.renameQualityMap(e2.qualitys);
                    cell.subtitles = e2.subtitles;
                    call();
                  }, function (e) { cell.url = ''; call(); });
                },
        ...
        element.loading = false;
        if (ctx && ctx.source) OnlineMod.Stats.hit(ctx.source, false);
        try {
          if (OnlineMod && OnlineMod.UI && OnlineMod.UI.errorModal) {
            OnlineMod.UI.errorModal('Нет ссылки', OnlineMod.UI._buildErrorHint ? OnlineMod.UI._buildErrorHint(e) : 'Источник не дал поток');
          } else {
    ...
    component.contextmenu({
      ...
      file: function (call) {
        ctx.getStream(element, function (el) {
          call({ file: el.stream, quality: el.qualitys });
        }, function () {
          Lampa.Noty.show(Lampa.Lang.translate('online_mod_nolink'));
        });
      }
...
(function injectOnlineModStyle(){
  if (window.__online_mod_style_v2) return;
  window.__online_mod_style_v2 = true;

  function tryInject(){
    try{
      if (typeof $ === 'undefined' || !$('head').length) return false;
      var css = `
        .online_mod_card{
          position: relative;
          border-radius: 14px;
        ...
        .online_mod_card:hover{
          transform: scale(1.01);
          background: rgba(255,255,255,0.06);
          border-color: rgba(255,255,255,0.10);
        }
        .online_mod_card--viewed{
        ...
        .online_mod_modal__title{
          font-size: 18px;
          font-weight: 700;
          margin-bottom: 8px;
        }
        .online_mod_modal__text{
          font-size: 13px;
          line-height: 1.35;
          opacity: .85;
        }
        .online_mod_modal__btns{
          margin-top: 14px;
          display:flex;
          justify-content:flex-end;
        }
        .online_mod_modal__btn{
          padding: 10px 14px;
          border-radius: 10px;
          background: rgba(255,255,255,0.06);
          border: 1px solid rgba(255,255,255,0.10);
        }
        .online_mod_modal__btn:hover{
          background: rgba(255,255,255,0.10);
        }
      `;
      $('head').append('<style id="online_mod_style_v2">'+css+'</style>');
      return true;
  ...
  if(!tryInject()){
    ...
    setTimeout(function(){ try{ clearInterval(t); }catch(e){} }, 8000);
  }
})();

// ================= END ONLINE MOD V2 CORE =================
// ================= ONLINE MOD V2 STEP3 HELPERS =================
OnlineMod.UI = OnlineMod.UI || {};

// Safe DOM helper: wait for jQuery and head
OnlineMod.UI._whenReady = function (cb, tries) {
  tries = tries == null ? 120 : tries;
  if (typeof $ !== 'undefined' && $('head').length) return cb();
  if (tries <= 0) return;
  setTimeout(function () { OnlineMod.UI._whenReady(cb, tries - 1); }, 50);
};

// Skeleton loader (lightweight placeholders while provider loads)
OnlineMod.UI.showSkeleton = function (component, count) {
  try {
    if (!component || !component.append) return;
    count = count || 8;
    component.reset();
    for (var i = 0; i < count; i++) {
      var node = $('<div class="online_mod_card online_mod_skel">' +
        '<div class="online_mod_skel__line online_mod_skel__line--w1"></div>' +
        '<div class="online_mod_skel__line online_mod_skel__line--w2"></div>' +
        '<div class="online_mod_skel__bar"></div>' +
      '</div>');
...
// Pretty error modal (polished)
OnlineMod.UI.errorModal = OnlineMod.UI.errorModal || function(title, message){
  title = title || 'Источник недоступен';
  message = message || 'Попробуйте позже или смените зеркало/прокси.';

  var html = $(
    '<div class="online_mod_modal">' +
      '<div class="online_mod_modal__title"></div>' +
      '<div class="online_mod_modal__text"></div>' +
      '<div class="online_mod_modal__btns">' +
        '<div class="online_mod_modal__btn online_mod_modal__btn--ok">OK</div>' +
...
// Stats modal (optional UI – can be wired later)
OnlineMod.UI.showStatsModal = function () {
  try {
    var stats = (OnlineMod.Stats && OnlineMod.Stats.get ? OnlineMod.Stats.get() : {src:{}}) || {src:{}};
    var src = stats.src || {};
    var keys = Object.keys(src);

    var html = $('<div class="online_mod_modal online_mod_stats"></div>');
    html.append('<div class="online_mod_modal__title">Статистика источников</div>');

    if (!keys.length) {
      html.append('<div class="online_mod_modal__text">Пока нет данных. Запустите пару серий/переводов.</div>');
    } else {
      var list = $('<div class="online_mod_stats__list"></div>');
      keys.sort().forEach(function (k) {
        var ok = src[k].ok || 0;
        var err = src[k].err || 0;
        var total = ok + err;
        var pct = total ? Math.round((ok / total) * 100) : 0;
        var row = $('<div class="online_mod_stats__row"></div>');
        row.append('<div class="online_mod_stats__name">' + String(k).toUpperCase() + '</div>');
        row.append('<div class="online_mod_stats__meta">' + ok + ' ok / ' + err + ' err</div>');
        row.append('<div class="online_mod_stats__pct">' + pct + '%</div>');
        list.append(row);
      });
      html.append(list);
    }

    html.append('<div class="online_mod_modal__btns"><div class="online_mod_modal__btn online_mod_modal__btn--ok">OK</div></div>');

    var modal = new Lampa.Modal();
    modal.open({ title: '', html: html });
    html.find('.online_mod_modal__btn--ok').on('hover:enter', function(){ modal.close(); });
  } catch (e) {}
};

// Inject additional styles (skeleton + modal polish)
OnlineMod.UI._whenReady(function(){
  if (window.__online_mod_style_v3) return;
  window.__online_mod_style_v3 = true;
  ...
  var css = `
    ...
    /* Skeleton */
    .online_mod_skel{ padding: 14px 14px 12px; }
    .online_mod_skel__line{
      height: 10px;
      border-radius: 999px;
    ...
    .online_mod_skel__line:after{
      ...
      animation: online_mod_shimmer 1.2s infinite;
    }
    .online_mod_skel__line--w1{ width: 55%; }
    .online_mod_skel__line--w2{ width: 35%; opacity: .85; }
    .online_mod_skel__bar{
      height: 2px;
      width: 100%;
      border-radius: 999px;
      background: rgba(255,255,255,0.06);
    }
    @keyframes online_mod_shimmer{
      0%{ left:-50%; }
      100%{ left:120%; }
    }
    ...
    .online_mod_chips{ display:flex; gap:8px; flex-wrap:wrap; }
    .online_mod_chip{
      padding: 8px 10px;
      border-radius: 999px;
    ...
    .online_mod_chip:hover{
      background: rgba(255,255,255,0.08);
      border-color: rgba(255,255,255,0.16);
    }
    .online_mod_chip--active{
      background: rgba(16,163,127,0.20);
      border-color: rgba(16,163,127,0.35);
      opacity: 1;
    }
    .online_mod_stats_btn{
      width: 34px;
      height: 34px;
      border-radius: 999px;
      display:flex;
    ...
    .online_mod_stats_btn:hover{
      background: rgba(255,255,255,0.08);
      border-color: rgba(255,255,255,0.16);
      opacity: 1;
    }
    .online_mod_viewed_dot{
      position:absolute;
      left: 10px;
      top: 10px;
      width: 8px;
    ...
    .online_mod_progress__bar{
      height: 100%;
      background: rgba(16,163,127,0.95);
    }

    /* Compact tweaks */
    .online_mod_card{ margin: 6px 0; }
    .online_mod_badge{ top:8px; right:8px; padding:3px 7px; font-size:10px; }
    .online_mod_viewed_dot{ left:8px; top:8px; width:7px; height:7px; box-shadow: 0 0 0 3px rgba(16,163,127,0.14); }
    .online_mod_chip{ padding: 7px 10px; font-size: 12px; }
    .online_mod_stats_btn{ width: 32px; height: 32px; }
    ...
    .online_mod_chips::-webkit-scrollbar{ display:none; }
    .online_mod_header{
      position: relative;
    }
    .online_mod_header:before,
    .online_mod_header:after{
      content:'';
      position:absolute;
      top:0; bottom:0;
      width: 18px;
    ...
    .online_mod_header:before{
      left:0;
      background: linear-gradient(90deg, rgba(15,15,15,1), rgba(15,15,15,0));
    }
    .online_mod_header:after{
      right:40px;
      background: linear-gradient(270deg, rgba(15,15,15,1), rgba(15,15,15,0));
    }

/* Stats */
    .online_mod_stats__list{ margin-top: 10px; display:flex; flex-direction:column; gap:8px; }
    .online_mod_stats__row{
    ...
    .online_mod_stats__name{ font-weight: 700; letter-spacing:.06em; font-size: 12px; opacity: .95; }
    .online_mod_stats__meta{ font-size: 12px; opacity: .85; }
    .online_mod_stats__pct{ font-size: 12px; font-weight: 700; opacity: .95; }
  `;

  $('head').append('<style id="online_mod_style_v3">'+css+'</style>');
});

// ================= ONLINE MOD V2 STEP8 NET WRAP =================
(function(){
  if (window.__online_mod_netwrap_v1) return;
  window.__online_mod_netwrap_v1 = true;

  try {
    if (typeof Lampa !== 'undefined' && Lampa.Reguest && Lampa.Reguest.prototype) {
      var _silent = Lampa.Reguest.prototype.silent;
      if (typeof _silent === 'function' && !Lampa.Reguest.prototype.__online_mod_patched) {
        Lampa.Reguest.prototype.__online_mod_patched = true;
        Lampa.Reguest.prototype.silent = function(url, success, error, nocache, params){
          // Wrap error to always provide xhr/status (real status, not string parsing)
          var errWrap = error;
          if (typeof error === 'function') {
            errWrap = function(xhr, msg){
              try {
                if (xhr && typeof xhr.status !== 'number' && xhr.xhr && typeof xhr.xhr.status === 'number') xhr.status = xhr.xhr.status;
              } catch(e){}
              return error(xhr, msg);
            };
          }
          return _silent.call(this, url, success, errWrap, nocache, params);
        };
      }
    }
  } catch(e){}
})();
// ================= END ONLINE MOD V2 STEP8 NET WRAP =================

// ================= END ONLINE MOD V2 STEP3 HELPERS =================
    ...
    function lumex(component, _object) {
      ...
      this.search = function (_object, kinopoisk_id, data) {
        try { if (OnlineMod && OnlineMod.UI && OnlineMod.UI.showSkeleton) OnlineMod.UI.showSkeleton(component, 8); } catch(e) {}
      ...
      function append(items) {
        OnlineMod.UI.renderItems(component, object, items, {
          source: 'rezka2',
          select_title: select_title,
          voice_name: (filter_items && filter_items.voice && filter_items.voice[choice.voice]) ? filter_items.voice[choice.voice] : '',
          chips: (function(){
            var list = [];
            if (filter_items && filter_items.season && filter_items.season.length) {
              var t = filter_items.season[choice.season] || filter_items.season[0] || 'Сезон';
              list.push({ text: String(t), active: true, onClick: function(){
                  try {
                    choice.season = (choice.season + 1) % filter_items.season.length;
                    component.reset();
                    filter();
                    append(filtred());
                    component.saveChoice(choice);
                  } catch (e) {
                    try{ component.openFilter && component.openFilter(); }catch(_e){}
                  }
                } });
            }
            if (filter_items && filter_items.voice && filter_items.voice.length) {
              var vt = filter_items.voice[choice.voice] || filter_items.voice[0] || 'Озвучка';
              list.push({
                text: String(vt),
                active: true,
                onClick: function(){
                  try {
                    choice.voice = (choice.voice + 1) % filter_items.voice.length;
                    if (choice.voice_name != null) choice.voice_name = filter_items.voice[choice.voice];
                    component.reset();
                    filter();
                    append(filtred());
                    component.saveChoice(choice);
            ...
            list.push({ text: 'СТАТИСТИКА', active: false, onClick: function(){ try{ OnlineMod.UI.showStatsModal(); }catch(e){} } });
            return list;
          })(),
          getStream: function (element, ok, err) {
            getStream(element, ok, err);
          }
        });
      }
    }
    ...
    function rezka2(component, _object) {
      ...
      function append(items) {
        ...
        items.forEach(function (element) {
          ...
          var view = Lampa.Timeline.view(hash);
          var item = Lampa.Template.get('online_mod', element);
          try{ item.addClass && item.addClass('online_mod_card'); }catch(e){}
...
// ===== OVERRIDE KP CACHE WITH LRU =====
(function(){
  if(typeof OnlineMod === 'undefined') return;
  var kp_cache_v2 = new OnlineMod.Cache.LRU(120, 1000 * 60 * 60);

  getCache = function(key){ return kp_cache_v2.get(key); };
  setCache = function(key, value){ kp_cache_v2.set(key, value); };

  getFromCache = function(method, oncomplite, onerror){
    var json = kp_cache_v2.get(method);
    if(json){
...
// Expose stats modal for quick access (console): online_mod_stats()
try { window.online_mod_stats = function(){ if (OnlineMod && OnlineMod.UI && OnlineMod.UI.showStatsModal) OnlineMod.UI.showStatsModal(); }; } catch(e) {}
