(function() {
  ...
  (function() {
    ...
    var AJAX_TIMEOUT = 5500;
    var CONCURRENCY = 3;
    var RETRY_COUNT = 1;
    var RETRY_DELAY_MS = 900;
    var SWR_WINDOW = 60 * 1e3;
      ...
      return new Promise(function(resolve) {
        function next() {
          while (active < limit && i < tasks.length) {
            (function(idx) {
              active++;
              Promise.resolve().then(tasks[idx]).then(function(r) {
                results[idx] = r;
              }).catch(function(e) {
                results[idx] = e;
              }).finally(function() {
      ...
      return new Promise(function(resolve) {
        $.ajax({
          ...
          success: function(_resp, _text, xhr) {
            resolve({ ok: true, xhr: xhr });
          },
          error: function(xhr) {
            resolve({ ok: false, xhr: xhr });
          }
        });
      });
    }
    ...
    function checkAlive(parsers) {
      ...
      return runPool(tasks, CONCURRENCY).then(function() {
        return results;
      });
    }
    ...
    function buildItem(parser) {
      var html = '<div class="pubtorr-parser-modal__item selector status-unknown" data-parser-id="' + parser.id + '"><div class="pubtorr-parser-modal__info"><div class="pubtorr-parser-modal__name">' + parser.name + '</div></div><div class="pubtorr-parser-modal__status"></div></div>';
      var item = $(html);
    ...
    function openParserModal() {
      ...
      var modal = $(
        '<div class="pubtorr-parser-modal"><div class="pubtorr-parser-modal__head"><div class="pubtorr-parser-modal__current"><div class="pubtorr-parser-modal__current-label">' + Lampa.Lang.translate("lme_parser_current") + '</div><div class="pubtorr-parser-modal__current-value"></div></div><div class="pubtorr-parser-modal__actions"><div class="pubtorr-parser-modal__meta"><div class="pubtorr-meta-line">' + Lampa.Lang.translate("lme_parser_version") + " " + PUBTORR_VERSION + " (" + PUBTORR_BUILD + ')</div><div class="pubtorr-meta-line"><span class="pubtorr-lastcheck-label">' + Lampa.Lang.translate("lme_parser_lastcheck") + '</span> <span class="pubtorr-lastcheck-value">—</span></div></div><div class="pubtorr-parser-modal__action selector">' + Lampa.Lang.translate("lme_parser_refresh") + '</div></div></div><div class="pubtorr-parser-modal__list"></div><div class="pubtorr-parser-modal__legend"><div class="pubtorr-parser-modal__legend-item status-ok">' + Lampa.Lang.translate("lme_parser_status_ok") + '</div><div class="pubtorr-parser-modal__legend-item status-auth-error">' + Lampa.Lang.translate("lme_parser_status_auth") + '</div><div class="pubtorr-parser-modal__legend-item status-network-error">' + Lampa.Lang.translate("lme_parser_status_network") + '</div><div class="pubtorr-parser-modal__legend-item status-unknown">' + Lampa.Lang.translate("lme_parser_status_unknown") + "</div></div></div>"
      );
      ...
      try {
        healthEnabled = Lampa.Storage.get(HEALTH_KEY, true);
      } catch (e) {
        healthEnabled = true;
      }
    ...
    function injectStyles() {
      ...
      Lampa.Template.add("pubtorr_style", "<style>.pubtorr-parser-modal{--pubtorr-status-ok:#19c37d;--pubtorr-status-auth:#ff4d4f;--pubtorr-status-network:#ff4d4f;--pubtorr-status-unknown:#8c8c8c;--pubtorr-status-checking:#f5a623;--pubtorr-selected-border:#fff;display:flex;flex-direction:column;gap:1em}.pubtorr-parser-modal__head{display:flex;align-items:flex-start;justify-content:space-between;gap:1em}.pubtorr-parser-modal__current-label{font-size:.9em;opacity:.7}.pubtorr-parser-modal__current-value{font-size:1.1em}.pubtorr-parser-modal__actions{display:flex;align-items:flex-start;gap:.8em}.pubtorr-parser-modal__meta{display:flex;flex-direction:column;gap:.25em;opacity:.75;font-size:.85em}.pubtorr-parser-modal__action{padding:.5em .9em;border-radius:.6em;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2)}.pubtorr-parser-modal__action.focus{border-color:var(--pubtorr-selected-border)}.pubtorr-parser-modal__list{display:flex;flex-direction:column;gap:.6em}.pubtorr-parser-modal__item{position:relative;display:flex;align-items:center;justify-content:space-between;gap:1em;padding:.8em 1em .8em 1.8em;border-radius:.7em;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);overflow:hidden}.pubtorr-parser-modal__item::before{content:'';position:absolute;left:.8em;top:50%;width:.55em;height:.55em;border-radius:50%;background:var(--pubtorr-status-color,var(--pubtorr-status-unknown));transform:translateY(-50%);box-shadow:0 0 .6em rgba(0,0,0,0.3)}.pubtorr-parser-modal__item.is-selected,.pubtorr-parser-modal__item.focus{border-color:var(--pubtorr-selected-border)}.pubtorr-parser-modal__info{display:flex;flex-direction:column;gap:.25em;min-width:0}.pubtorr-parser-modal__name{font-size:1em}.pubtorr-parser-modal__status{font-size:.8em;opacity:.7;text-align:right;align-self:center}.pubtorr-parser-modal__legend{display:flex;flex-wrap:wrap;gap:.8em 1.2em;font-size:.85em;opacity:.7}.pubtorr-parser-modal__legend-item{position:relative;padding-left:1.2em}.pubtorr-parser-modal__legend-item::before{content:'';position:absolute;left:0;top:.55em;width:.5em;height:.5em;border-radius:50%;background:var(--pubtorr-status-color,var(--pubtorr-status-unknown))}.pubtorr-parser-modal__item.status-ok,.pubtorr-parser-modal__legend-item.status-ok{--pubtorr-status-color:var(--pubtorr-status-ok)}.pubtorr-parser-modal__item.status-auth-error,.pubtorr-parser-modal__legend-item.status-auth-error{--pubtorr-status-color:var(--pubtorr-status-auth)}.pubtorr-parser-modal__item.status-network-error,.pubtorr-parser-modal__legend-item.status-network-error{--pubtorr-status-color:var(--pubtorr-status-network)}.pubtorr-parser-modal__item.status-unknown,.pubtorr-parser-modal__legend-item.status-unknown{--pubtorr-status-color:var(--pubtorr-status-unknown)}.pubtorr-parser-modal__item.status-checking{--pubtorr-status-color:var(--pubtorr-status-checking)}@media(max-width:600px){.pubtorr-parser-modal__head{flex-direction:column;align-items:flex-start}.pubtorr-parser-modal__actions{width:100%;justify-content:space-between}.pubtorr-parser-modal__item{flex-direction:column;align-items:flex-start}.pubtorr-parser-modal__status{text-align:left}}@keyframes pubtorrPulse{0%{transform:translateY(-50%) scale(.9);opacity:.55}50%{transform:translateY(-50%) scale(1.12);opacity:1}100%{transform:translateY(-50%) scale(.9);opacity:.55}}@keyframes pubtorrShimmer{0%{transform:translateX(-120%);opacity:0}15%{opacity:.9}60%{opacity:.9}100%{transform:translateX(140%);opacity:0}}@keyframes pubtorrDotsA{0%{content:''}25%{content:'.'}50%{content:'..'}75%{content:'...'}100%{content:'';}}.pubtorr-parser-modal__item.status-checking{border-color:rgba(245,166,35,.35)}.pubtorr-parser-modal__item.status-checking::before{animation:pubtorrPulse 1.05s ease-in-out infinite;box-shadow:0 0 0 .22em rgba(245,166,35,.16),0 0 .9em rgba(245,166,35,.35)}.pubtorr-parser-modal__item.status-checking::after{content:'';position:absolute;inset:0;background:linear-gradient(110deg,transparent 0%,rgba(255,255,255,.08) 35%,rgba(255,255,255,.14) 50%,rgba(255,255,255,.08) 65%,transparent 100%);transform:translateX(-120%);animation:pubtorrShimmer 1.1s ease-in-out infinite;pointer-events:none}.pubtorr-parser-modal__item.status-checking .pubtorr-parser-modal__status{position:relative;opacity:.85}.pubtorr-parser-modal__item.status-checking .pubtorr-parser-modal__status::after{content:'';display:inline-block;width:1.2em;margin-left:.15em;text-align:left;opacity:.8;animation:pubtorrDotsA 1.1s steps(1,end) infinite}@keyframes pubtorrResultFlashOk{0%{box-shadow:0 0 0 rgba(25,195,125,0)}30%{box-shadow:0 0 .9em rgba(25,195,125,.25)}100%{box-shadow:0 0 0 rgba(25,195,125,0)}}@keyframes pubtorrResultFlashBad{0%{box-shadow:0 0 0 rgba(255,77,79,0)}30%{box-shadow:0 0 .9em rgba(255,77,79,.22)}100%{box-shadow:0 0 0 rgba(255,77,79,0)}}.pubtorr-flash-ok{animation:pubtorrResultFlashOk .55s ease-out}.pubtorr-flash-bad{animation:pubtorrResultFlashBad .55s ease-out}</style>");
